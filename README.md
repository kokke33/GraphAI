...
### GraphAIの魅力
- Yaml形式のデータ定義のみでアプリケーション開発が可能（ローコード！）
- 複数の同期・非同期APIコールの管理における複雑さを軽減（GraphAIエンジンにお任せ！）
- プログラミング言語の専門知識不要

### 最近のLLMは利用料がかなり安いものがある
- ２００万文字で１００円しない
- 高性能で無料のモデルもある
- PCやスマホで稼働するLLMも誕生していきLLM常時接続が当たり前になっていく
...
> GraphAIは、非同期データフロー実行エンジン（データを順番に処理するのではなく、同時に複数のデータを扱うことができる仕組み）であり、プログラミングの専門知識がない方 でも簡単にAIアプリケーションを作成できるオープンソースのツールです。<br><br>
...
このプロセスでは、データを取得し、解析し、最終的な回答を生成する一連のステップがありますが、GraphAIがこのすべてを自動的に管理し、先行関係や同期・非同期含め実行してくれます。<br><br>
